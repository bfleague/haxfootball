...
export function SafetyKickReturn({
    ...
    function leave(player: GameStatePlayer) {
        if (player.id === playerId) {
            if (isInMainField(player)) {
                ...
                $effect(($) => {
                    $.send({
                        message: t`🚪 ${player.name} left during the safety-kick return!`,
                        color: COLOR.WARNING,
                    });
                });
    ...
    function $handleTouchdown(frame: Frame) {
        ...
        $effect(($) => {
            $.send({
                message: t`🔥 Safety-kick return touchdown by ${frame.player.name}!`,
                color: COLOR.SUCCESS,
            });
    ...
    function $handleOutOfBounds(frame: Frame) {
        ...
        if (isCompletelyInsideMainField(frame.player)) {
            $effect(($) => {
                $.send({
                    message: t`🚪 ${frame.player.name} stepped out on the safety-kick return.`,
                    color: COLOR.WARNING,
                });
